...
namespace MyAnimeViewer.Utility
{
    public class BackupManager
    {
        ...
        private static readonly string[] Files = { "config.xml" };
        ...
        public static void Run()
        {
            ...
            {
                Log.Error("Error deleting old backup: " + e);
            }
            ...
            finally
            {
                if(restored)
                {
                    ErrorManager.AddError(file + " was corrupted but restored from the latest backup.", 
                        ...
                        + "Backups are generated on the first start of each daym so there may be lost data." + Environment.NewLine
